...
{
    self.dataSource = [NSMutableArray array];
    XMPPMessageArchivingCoreDataStorage *storage = [XMPPManager sharedInstance].xmppMessageArchivingCoreDataStorage;
    ...
    if (fetchedObjects != nil) {
        ...
        for (int i=0; i<fetchedObjects.count; i++)
        {
            ...
            [dataDic setObject:fromOthers?recordMessage.bareJidStr:[XMPPManager sharedInstance].myJID.user forKey:@"strName"];
            [dataDic setObject:fromOthers ? otherStr:myStr forKey:@"strIcon"];
            
            //type:voice text picture
            NSNumber  *type;
            NSLog(@"message subject");
            NSLog(@"%@", recordMessage.message.subject);
            
            if ([recordMessage.message.subject isEqualToString:@"voice"]) {
                
                type = @(2);
                NSLog(@"%@", recordMessage.body);
