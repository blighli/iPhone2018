...
#pragma mark 查询
- (void)requestProductWithId:(NSString *)productId with:(NSDictionary *)dic{
    ...
    if (self.goodsRequestFinished) {
        ...
        if ([SKPaymentQueue canMakePayments]) { //用户允许app内购
            ...
            if (productId.length) {
                ...
                hud.graceTime = NetworkRequestGraceTimeTypeNormal;
...
-(void)saveReceipt {
    ...
    NSString *userToken = [[MyUserInfo getUserInfo] objectForKey:SHInvokerUserInforuser_token];
    ...
    NSMutableDictionary *dic =[NSMutableDictionary dictionaryWithObjectsAndKeys:
                        ...
                        nil];
    [dic setObject:[self.dataDic objectForKey:@"ios_cost"] forKey:@"ios_cost"];
    [dic setObject:[self.dataDic objectForKey:@"cost_type"] forKey:@"cost_type"];
...
//                                     @"receipt-data": [dic objectForKey:receiptKey]
//                                     };
//    NSError *error;
//    // 转换为 JSON格式
...
//                          }];
    ...
    [params setObject:[dic objectForKey:userIdKey] forKey:@"token"];
    [params setObject:[dic objectForKey:@"cost_type"] forKey:@"cost_type"];
    [params setObject:[dic objectForKey:receiptKey] forKey:@"trade_no"];
    [params setObject:[dic objectForKey:@"ios_cost"] forKey:@"total_fee"];
    [params setObject:[dic objectForKey:dateKey] forKey:@"notify_time"];
    

    [AFNetInterFaceManager Post:GETIsopay parameters:params graceTime:NetworkRequestGraceTimeTypeNone success:^(NSDictionary *response, NSURLSessionDataTask *task) {
        NSLog(@"%@",response);
        if ([[response objectForKey:@"status"] integerValue]==0000) {
             [self removeReceipt];
        }else if ([[response objectForKey:@"msg"] isEqualToString:@"该订单已经支付成功"]){
            [self removeReceipt];
        }
