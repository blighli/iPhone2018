...
#import "PDPocketDiaryEditView.h"
...
@interface PDPieceEditViewController () <PDPieceEditToolbarDelegate, PDImagePickerControllerDelegate, PDPocketDiaryEditViewDelegate, PDDisplayPhotoViewControllerDelegate, PDQuestionEditViewControllerDelegate>

@property (nonatomic, weak) IBOutlet PDPieceEditToolbar *toolbar;
@property (nonatomic, weak) IBOutlet PDPocketDiaryEditView *editView;
...
- (void)setupEditView:(PDPocketDiaryEditView *)editView withIndex:(NSInteger)index
{
    PDPieceCellData *data = self.dataArray[index];
    [self setupEditView:editView withData:data];
}

- (void)setupEditView:(PDPocketDiaryEditView *)editView withData:(PDPieceCellData *)data
{    
...
{
    // 添加两个临时的视图来展示滑动动画
    PDPocketDiaryEditView *slideInEditView = [self getSlideInEditViewWithType:type];
    PDPocketDiaryEditView *slideOutEditView = [self getSlideOutEditViewWithType:type];
...
- (PDPocketDiaryEditView *)getSlideInEditViewWithType:(EditPieceCellChangeType)type
{
    // 滑入的view
...
- (PDPocketDiaryEditView *)getSlideOutEditViewWithType:(EditPieceCellChangeType)type
{
    // 滑出的view
...
- (PDPocketDiaryEditView *)createEditViewWithFrame:(CGRect)frame cellData:(PDPieceCellData *)data
{
    PDPocketDiaryEditView *editView = [[PDPocketDiaryEditView alloc] initWithFrame:frame];
    NSArray *nibArray = [[UINib nibWithNibName:@"PDPocketDiaryEditView" bundle:nil] instantiateWithOwner:editView options:nil];
...
#pragma mark - PDPocketDiaryEditView
